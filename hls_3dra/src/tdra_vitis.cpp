...
extern "C"
{
    ...
    // #pragma HLS INTERFACE s_axilite port = return bundle = control
        ...
        //sending instructions to 3DRA
        for (int i = 0; i < size; i += PE_COUNT)
        {
            //currently need to manually connect PE instructions to the respective PE
            ...
            TDRA(in_channel, in1_local, in2_local, opcode_local, out_local);
            in_channel[i] = out_local[0];
            in_channel[i + 1] = out_local[1];
            out[i] = out_local[0];
            out[i + 1] = out_local[1];
        }
    }
